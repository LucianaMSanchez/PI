...
import { ORDER_DEF_SUCCESS, ORDER_DEF_FAILURE, ORDER_SPEED_SUCCESS, ORDER_SPEED_FAILURE, FILTER_ORIGIN_SUCCESS, FILTER_ORIGIN_FAILURE, ADD_CHAMPS_SUCCESS, ADD_CHAMPS_FAILURE, CLEAR_CHAMPS } from "./action.types";
...
export const clearChamps = () => {
  return { type: CLEAR_CHAMPS };
};


export const addChamps = (id) => {
  const endpoint = "http://localhost:3001/champs/champs";
  return async (dispatch) => {
    try {
       const {data} = await axios.post(endpoint, { id })
       return dispatch({
